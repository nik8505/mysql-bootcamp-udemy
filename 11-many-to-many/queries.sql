...
WHERE series.id NOT IN(SELECT reviews.series_id FROM reviews);

-- this one is better, because we don't have to do a subquery
SELECT series.title AS unreviewed_series
FROM series 
LEFT JOIN reviews ON series.id = reviews.series_id 
...
SELECT 
    ...
    IFNULL(AVG(reviews.rating), 0) AS AVERAGE, 
	CASE 
...
SELECT series.title, reviews.rating, CONCAT(reviewers.first_name, ' ', reviewers.last_name) AS reviewer
FROM series 
